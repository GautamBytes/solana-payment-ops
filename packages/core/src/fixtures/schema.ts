...
const transactionMessageSchema = z
  .object({
    header: z
      .object({
    ...
    addressTableLookups: z.array(addressTableLookupSchema).default([]),
...
const rpcTransactionEnvelopeBaseSchema = z
  .object({
...
type RpcTransactionEnvelopeBase = z.infer<
  typeof rpcTransactionEnvelopeBaseSchema
>;

function validateEnvelope(
  rpcTransaction: RpcTransactionEnvelopeBase,
  ...
  if (
    ...
    issue(["version"], "Legacy transactions cannot declare loaded addresses");
  }

  const resolvedAccountCount =
    message.accountKeys.length + loadedWritable.length + loadedReadonly.length;
  ...
  for (const [groupIndex, group] of (meta.innerInstructions ?? []).entries()) {
  ...
  for (const [balanceKind, balances] of [
    ...
    balances.forEach((balance, balanceIndex) => {
      if (balance.accountIndex >= resolvedAccountCount) {
        issue(
          ["meta", balanceKind, balanceIndex, "accountIndex"],
...
export const RpcTransactionEnvelopeSchema =
  rpcTransactionEnvelopeBaseSchema.superRefine(validateEnvelope);

const paymentExpectationSchema = z
  .object({
...
export type RpcTransactionEnvelope = z.infer<
  typeof RpcTransactionEnvelopeSchema
>;
export type PaymentFixture = z.infer<typeof PaymentFixtureSchema>;
export type CompiledInstruction =
  RpcTransactionEnvelope["transaction"]["message"]["instructions"][number];
